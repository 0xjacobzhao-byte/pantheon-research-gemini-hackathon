"""FastAPI application — Pantheon Research Gemini Hackathon Demo."""
...
app = FastAPI(
    title="Pantheon Research — Gemini Hackathon Demo",
    description="Gemini-powered investment research overlay with dual-provider comparison (Qwen vs DeepSeek as secondary)",
    version="1.0.0",
)
...
async def root():
    return {
        "project": "Pantheon Research — Gemini Hackathon Demo",
        "version": "1.0.0",
        "demo_mode": DEMO_MODE,
        "docs": "/docs",
    }
...
@app.get("/api/overlay/gemini/{ticker}")
async def overlay_gemini(ticker: str):
    try:
        evidence = load_evidence(ticker)
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail=f"No evidence data for ticker: {ticker}")
    overlay = await run_gemini_overlay(evidence)
    return overlay
...
async def demo_flow():
    return {
        "title": "Pantheon Research — Gemini Hackathon Demo Flow",
        "steps": [
            {
                "step": 1,
                "title": "Select Ticker",
